...
sat18$aa = ifelse(sat18$kp %in% kpd[1:2], "kevad1",
                  ifelse(sat18$kp %in% kpd[3:7], "kevad2",
                         ifelse(sat18$kp %in% kpd[8], "sygis1", "sygis2")))
...
# #save(data, file = "data_glmer.RData")

#võtab mõned tunnid aega, seetõttu panen tulemused kaasa
...
ns1 = grep("sygis1", names1, value = T) #sügis 1
