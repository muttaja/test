...
#NIR-id ja SWIR-id ei lange p‰ris kokku, aga mis seal ikka...

#kuup‰evad mudeli jaoks: 

# 2018-05-10

# siin kasutame LC kevad2 prognoosv‰‰rtusi?
# 2018-05-26 33840      0      0
# 2018-05-27     0  50760      0
# 2018-05-28 33840 
...
#see ju vigane, peab olema cat pıhjal!!!
...
ww0 = rbind(wide1,wide2) #,wide3, #‰kki see kolmas rikub ‰ra?
...
#kas lm ja lmer, kus piksli mıju on juhuslik, kuna pikslid ei kattu, annavad erineva tulemuse?
...
#tahan prognoosida sentineli kanali v‰‰rtust lansati kanalite pıhjal:
#erindid vıivad tekkida sellest, et pikslid on erineva suurusega?
#eeldame, et v‰iksema piksilga sentinel on "tıde", ehk landsat on mııdetud veaga?
#how do deal with measurement uncerainty?

############### v‰ga lihtsad mudelid ###############

#kas vıtta ainult mets?
ww = ww[ww$aproovitykk_id %in% mets_id,]


#jagame l‰bi vastavalt piksi suuruse erinevusega???
...
#seetıttu ka random effecte vaja
...
#n¸¸d saaks 2017 aasta info ka kasutusele vıtta?
...
mdls = list(m2, m3, m4, m5, m6, m7, m2l, m3l, m4l, m5l, m6l, m7l) #lineaarfunktsioon + cati juhuslik mıju
...
max(na.omit(data$predB02-data$predB02l)) #id ja cat annavad sama tulemuse, tıen‰oliselt optimeerimisel erinevus
#linear ja juhuslik mıju siiski erinev nats! 6.838033 tulex max erinevus, kui enne on landsati v‰‰rtused kohendatud sentineli piksli suurustele
...
#teisel katsel... n¸¸d rmse muidugi, aga lineaarne parem!
# [1] 55.10601
# [1] 53.90756
# > ms3;ms3l
# [1] 67.1406
...
#nonii, kohe n‰ha, et erinevad satid annavad erineva tulemuse
#sentineli trajektoorid mılemal kuup‰eval samad, aga landsat?
...
#aga kuidas n¸¸d p‰rast teisendust sentineli ja landsati kanalid neil kahel kuup‰eval plottuvad?
...
mean(c(ms2,ms3,ms4,ms8,ms11,ms12)) #11902.72 mudelil, mis ainult metsa pıhjal
...
#vaja oleks keskmisi erinevuse ¸le kuup‰evade
...
#asendame puuduvad v‰‰rtused prognoosidega
...
#
#kui n¸¸d agregeerida nii, et arvutab iga id kohta kıikide kanalite keskmise varieervuse ¸le "cat", siis see oleks inforks homogeensuse kohta!?
...
require(reshape2)
#NB! see homo tˆˆtab vaid selle andmestiku peal, kus on iga eraldise kohta mitu piksli v‰‰rtust
...
#n¸¸d nii kokku agregeerida, et vıtta uuesti arvesse erinevate bandide var.coeffid

nmh = names(homo); nmh = nmh[nmh != "B10"]; homo = homo[,nmh]
hh1 = homo %>% group_by(aproovitykk_id) %>%  summarise_all(funs(mean(.,na.rm = T)))
#jei, siit saabki v‰lja vıtta juba :)
#B10 pole vaja
...
#logaritmitud skooride histogramm? siis tuleks ka ebanormaalselt v‰iksed v‰‰rtused v‰lja?
...
#praegu ilma trajektoorita. landsati pıhjal prognoositud v‰‰rtused vıtta ka eraldi "trajektooriks"?

#mudel silumiseks ja puuduvate v‰‰rtuste kırvaldamiseks:
...
#ID*aastaeg ei tˆˆta tıen‰oliselt
#kuidas mudel selle olukorra lahendab? - vıtab ¸ldkeskmise
...
d5 = d5[order(d5$aa),]
#mnjaa, siin k¸ll astakudefitsiit...
